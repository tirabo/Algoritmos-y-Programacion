...
def datos_usuario(dominio, card_number, surname=""):
    ...
    if len(surname) > 0:
        response = requests.get(
            url,
            auth=(usuario, passwd),
            params={"cardnumber": card_number, "surname": surname},
        )
    else:
        response = requests.get(
            url, auth=(usuario, passwd), params={"cardnumber": card_number}
        )
    ...
    if response.status_code == 200:
        ...
        print(
            "Hubo un problema al cargar el archivo JSON del usuario con cardnumber",
            card_number,
            "en",
            dominio,
            "(status code:",
...
def prestamos_usuario(dominio, card_number, surname=""):
    ...
    if len(socio) == 0:
        ...
        response = requests.get(
            url, auth=(usuario, passwd), params={"patron_id": patron_id}
        )
        if response.status_code == 200:
            # El archivo JSON fue encontrado y cargado correctamente
            data = response.json()
            ...
            print(
                "Hubo un problema al cargar el archivo JSON de préstamos del usuario con cardnumber",
                card_number,
                "en",
                dominio,
                "(status code:",
...
def listar_bibliotecas(dominio):
    ...
    if response.status_code == 200:
        ...
        print(
            "Hubo un problema al cargar el archivo JSON de bibliotecas en",
            dominio,
            "(status code:",
            str(response.status_code) + ")",
        )
...
def datos_item(dominio, item_id):
    ...
    if response.status_code == 200:
        ...
        print(
            "Hubo un problema al cargar el archivo JSON del item con itemnunber",
            item_id,
            "en",
            dominio,
            "(status code:",
...
def main():
    dominios = [
        "https://faud.biblioadmin.unc.edu.ar",
        "https://eco.biblioadmin.unc.edu.ar",
        "https://ffyh.biblioadmin.unc.edu.ar",
        "https://agro.biblioadmin.unc.edu.ar",
        "https://famaf.biblioadmin.unc.edu.ar",
    ]
